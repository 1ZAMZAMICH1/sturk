...
// 🟢 ИСПРАВЛЕННЫЙ ДЕТЕКТОР: ПК всегда HIGH
...
const MemoizedClouds = memo(() => {
  ...
  const isBudget = perf === 'low';
  const cloudsRef = useRef();

  // Заморозка матриц для бюджета (совет из чата)
  useEffect(() => {
    if (isBudget && cloudsRef.current) {
  ...
  const cloudConfig = isBudget ? (
    <>
      {/* 🟢 БЮДЖЕТНАЯ ВЕРСИЯ: Та же композиция, но меньше сегментов */}
      <Cloud seed={10} segments={20} bounds={[15, 20, 1]} volume={60} color="#1a0b05" position={[0, 0, -18]} speed={0} opacity={1} />
      <Cloud seed={20} segments={15} bounds={[12, 18, 2]} volume={40} color="#2e1608" position={[0, 0, -14]} speed={0} opacity={0.95} />
      <Cloud seed={30} segments={12} bounds={[10, 15, 3]} volume={30} color="#542a0c" position={[0, 0, -10]} speed={0} opacity={0.85} />
      <Cloud seed={40} segments={10} bounds={[10, 12, 3]} volume={25} color="#783c12" position={[0, 0, -6]} speed={0} opacity={0.7} />
      <Cloud seed={50} segments={8} bounds={[8, 10, 2]} volume={20} color="#9c5219" position={[0, 0, -2]} speed={0} opacity={0.6} />
      <Cloud seed={60} segments={6} bounds={[8, 8, 2]} volume={15} color="#b86e28" position={[0, 0, 2]} speed={0} opacity={0.4} />
    </>
  ) : (
    <>
      {/* 🔴 ОРИГИНАЛЬНАЯ КОМПОЗИЦИЯ (БЕЗ ИЗМЕНЕНИЙ) */}
...
const HeroMobile = () => {
  ...
  const isBudget = detectPerformance() === 'low';
    ...
    <div className="hero-container-mob" style={{ overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, zIndex: 1, pointerEvents: 'none' }}>
        ...
        >
          ...
          <Sparkles count={isBudget ? 200 : 800} scale={[40, 30, 2]} position={[0, 0, 10]} size={2} speed={0.4} opacity={1} color="#ffcc66" noise={1} />
          <MemoizedClouds />
